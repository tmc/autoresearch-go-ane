...
func genStoriesSDPAForwardDynamicInference(dim, heads, seq int, residualScale float64) string {
	headDim := dim / heads
	scale := 1.0 / math.Sqrt(float64(headDim))
	spatial := seq + 1 + 4*dim
	...
	// Wo projection
	appendDynamicMatmulFP16(&b, "oo", "af", "Wo", dim, dim, seq)
	// Residual: out = x + scale*(oo - x) = (1-scale)*x + scale*oo
...
func genStoriesFFNForwardDynamicInference(dim, hidden, seq int, residualScale float64) string {
	spatial := seq + 1 + 3*hidden
