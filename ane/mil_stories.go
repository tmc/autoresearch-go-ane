...
func appendDynamicRMSNormFP16(b *strings.Builder, outVar, xVar, wVar string, dim, seq int) {
	...
	fmt.Fprintf(b, "        tensor<fp16, [1,1,1,%d]> %s_rrms = pow(x=%s_ss3,y=nhalf)[name=string(\"%s_rrms\")];\n", seq, outVar, outVar, outVar)
...
// genStoriesSDPAForwardDynamicInference generates an inference-only attention
// forward kernel. Same computation as the taps variant but outputs only x2
// (dim channels) instead of concat(x2, q, k, v, att_out) (5*dim channels).
func genStoriesSDPAForwardDynamicInference(dim, heads, seq int) string {
	headDim := dim / heads
	scale := 1.0 / math.Sqrt(float64(headDim))
	spatial := seq + 1 + 4*dim
	...
	fmt.Fprintf(&b, "        tensor<fp16, [1,%d,1,%d]> af = reshape(shape=att_shape,x=at)[name=string(\"af\")];\n", dim, seq)
	appendDynamicMatmulFP16(&b, "oo", "af", "Wo", dim, dim, seq)
	fmt.Fprintf(&b, "        tensor<fp16, [1,%d,1,%d]> out = add(x=x,y=oo)[name=string(\"out\")];\n", dim, seq)
	b.WriteString("    } -> (out);\n}\n")
	return b.String()
}

// genStoriesFFNForwardDynamicInference generates an inference-only FFN forward
// kernel. Same as taps variant but outputs only x_next (dim channels) instead
// of concat(x_next, h1) (dim+hidden channels).
func genStoriesFFNForwardDynamicInference(dim, hidden, seq int) string {
