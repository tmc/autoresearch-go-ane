...
3. **Read the in-scope files**: The repo is small. Read these files for full context:
   ...
   - `helpers.go` — fixed evaluation harness, data loading, random init. Do not modify.
...
**What you CANNOT do:**
- Modify `helpers.go`. It is read-only. It contains the evaluation harness and data loading.
- Modify `bench_test.go`. It is read-only.
- Add new Go dependencies. You can only use what's already imported.
- Modify the evaluation harness. The `evalLoss` function in `helpers.go` is the ground truth metric.
...
**NEVER STOP**: Once the experiment loop has begun, do NOT pause to ask the human if you should continue. Do NOT ask "should I keep going?" or "is this a good stopping point?". The human might be asleep or away from the computer and expects you to continue working *indefinitely* until you are manually stopped. You are autonomous. If you run out of ideas, think harder — re-read `experiment.go` and `helpers.go` for new angles, try combining previous near-misses, try more radical parameter changes. The loop runs until the human interrupts you, period.
